...
class ServerWebAction extends PageBase
{	
	...
	public function index()
	{
		...
		$arrTags=array('TabTagID'=>'ServerWeb','IP'=>'','Port'=>0,'ServID'=>0,'ClassName'=>'ServerWeb');
	...
	{
		$TypeID = '';
		foreach ($this->arrConfig['ServerTypeWeb'] as $val)
			$TypeID .= $val['TypeID'] . ',';
		...
		$arrParam['where']=' WHERE ServerType IN ('.$TypeID.') AND ServerType != 36' ;
	...
	public function showAddServerHtml()
	{
		...
		Utility::assign($this->smarty,$arrTags);
		$html = $this->smarty->fetch($this->arrConfig['skin'].'/YunWei/ServerWebEdit.html');
		$html=str_replace("</script>","<\/script>",str_replace("\r\n",'',$html));
	...
	public function setServerLocked()
	{
	    ...
	    if($ServerID && $ServerID>0)
	        $iResult = $this->objMasterBLL->setServerLocked($ServerID,$TypeID);
	    if($iResult==0)
	        $msg='';
	    else
