...
{	
	...
	{
		$arrParam['fields']='ID,UserName,RealName,Locked,JobNumber,CONVERT(VARCHAR(20),AddTime,120) AS AddTime,DeptID,AdminRole,SysUserName,BindAccount';
		...
		if($arrResult)
		{
			...
			$arrDeptList = $objMasterBLL->getDepartmentList();
			//角色列表
            $roleList = $objMasterBLL->getRoleList();
			...
			foreach ($arrResult as $val)
			{
				...
				if($arrDeptList)
				{
					...
					foreach ($roleList as $val3) {
					    if ($val['AdminRole'] == $val3['Id']) {
					        $arrResult[$iCount]['RoleName'] = $val3['RoleName'];
					        break;
                        }
                    }
				}				
    ...
    public function showSetRole()
    {
        ...
        $objMasterBLL = new MasterBLL();
        $roleList = $objMasterBLL->getRoleList();
        $arrTags = array('roleList' => $roleList, 'roleid' => $roleid, 'id' => $id);
        $send = array('Info'=>$arrTags);
        Utility::assign($this->smarty, $send);
        $html = $this->smarty->fetch($this->arrConfig['skin'].'/Yunwei/SysUserSetRole.html');
        $html=str_replace("</script>","<\/script>",str_replace("\r\n",'',$html));
        echo $html;
    }
    ...
    {
        $adminid = Utility::isNumeric('adminid',$_POST);
        $roleid = Utility::isNumeric('roleid',$_POST);
        $objMasterBLL = new MasterBLL();
        if (!$roleid || !$adminid) {
            echo -1;
        } else {
            $ret = $objMasterBLL->setUserRole($adminid, $roleid);
            echo $ret[0]['iResult'];
        }
    }
}
