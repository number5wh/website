...
class ServiceMinRoleAction extends PageBase
{
            ...
            if(isset($arrPassSec) && $arrPassSec)
            {
                ...
                foreach ($arrPassSec as $key => $val)
                {
                    ...
                    if(!empty($threedata[0]['changemoney']))
                        $arrRoleInfo['threeDayout'] = $threedata[0]['changemoney'];
                    else
                    ...
                    if(!empty($monthdata[0]['changemoney']))
                        $arrRoleInfo['monthDayout'] = $monthdata[0]['changemoney'];
                    else
                    ...
                    if(!empty($threedatain[0]['changemoney']))
                        $arrRoleInfo['threeDayin'] = $threedatain[0]['changemoney'];
                    else
                    ...
                    if(!empty($monthdatain[0]['changemoney']))
                        $arrRoleInfo['monthDayin'] = $monthdatain[0]['changemoney'];
                    else
                    ...
                    if(!empty($alldataout[0]['changemoney']))
                        $arrRoleInfo['allout'] = $alldataout[0]['changemoney'];
                    else
                    ...
                    if(!empty($alldatain[0]['changemoney']))
                        $arrRoleInfo['allin'] = $alldatain[0]['changemoney'];
                    else
                        $arrRoleInfo['allin'] =0;



                    $arrRoleInfo['HappyBeanMoney'] = $Role['iHappyBeanMoney'];
                    //用户账号锁定状态 0:有锁,-1:没锁
...
//		$userPower = $objUserDataBLL->getUserPowerInfo();
        ...
        $userBankInfo['TotalChargeMoney'] = Utility::FormatMoney($userBankInfo['TotalChargeMoney']);
...
//		}
        ...
        Utility::assign($this->smarty,$arrTags);
        $this->smarty->display($this->arrConfig['skin'].'/Service/RoleMinEdit.html');
    }
