...
class UserBankRateSuperAction extends PageBase
{	
	...
	public function index()
	{
		...
		$this->smarty->display($this->arrConfig['skin'].'/YunYing/UserBankRateSuperList.html');
	}	 
    ...
    public function getPagerUserBankRate($pagesize){
        ...
        $StartTime =date("Y-m-d",strtotime('-30 days'));// Utility::isNullOrEmpty('StartTime',$_POST)?$_POST['StartTime'] : date('Y-m-d');
        ...
        $SuperUser = 1;//Utility::isNumeric('SuperUser',$_POST)? $_POST['SuperUser'] : 0;  0普通玩家   1 商人
        ...
        for($t = $STime; $t <= $ETime;$t = strtotime('+1 day',$t)){
            ...
            if($isExits['ret']===1) {
                ...
                $strWhere .=" and RoleId  in (select Roleid from OM_MasterDB.dbo.T_SuperUser) and  TargetID  not in (select Roleid from OM_MasterDB.dbo.T_SuperUser)";
...
//                }
                $arrUserBankRateList[$iCount]['totalzz'] = Utility::FormatMoney($val['totalzz']);
                $arrUserBankRateList[$iCount]['totalMoney'] = Utility::FormatMoney($val['totalMoney']);
