...
class UserBankReturnDetailAction extends PageBase
{	
	...
	public function index()
	{
		...
		$this->smarty->display($this->arrConfig['skin'].'/YunYing/UserBankReturnDetailList.html');
	}	 
    ...
    public function getPagerUserBankRate($pagesize){
        ...
        for($t = $STime; $t <= $ETime;$t = strtotime('+1 day',$t)){
            ...
            //var_dump($subTable);
            if($rateType == 1){
                $strWhere = ' WHERE ChangeType = 6';//游戏转账类型 赠送
            } else {
...
//             }
	...
	public function getPagerUserBankReturnList()
	{
		...
		Utility::assign($this->smarty,$arrTags);
		$html = $this->smarty->fetch($this->arrConfig['skin'].'/YunYing/UserBankReturnDetailPage.html');
