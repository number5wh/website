...
class DayGameSortAction extends PageBase {
	...
	public function getPagerGameSortSort($pagesize) {
		...
		$where = "";
		if ($KindID) {
			$where .= " WHERE KindID = " . $KindID;
			if ($userId) {
				$where .= ' and RoleID = '.$userId;
			}
		} else {
			if ($userId) {
				$where .= ' WHERE RoleID = '.$userId;
			}
		}
		...
		$arrParam ['tableName'] = '(SELECT RoleID,RoleName,KindID,SUM(Money) as Money,SUM(CASE WHEN ChangeType=0 THEN 1 ELSE 0 END) as WinCount,SUM(CASE WHEN ChangeType=1 THEN 1 ELSE 0 END) as LostCount,
                            SUM(CASE WHEN ChangeType=2 THEN 1 ELSE 0 END) as DrawCount,SUM(CASE WHEN ChangeType=3 THEN 1 ELSE 0 END) as FleeCount,CONVERT(VARCHAR(100),MAX(AddTime),120) as UpdateTime' . ' FROM T_UserGameChangeLogs_' . date ( 'Ymd', strtotime ( $AddTime ) ) . $where .' GROUP BY RoleID,RoleName,KindID) T';
			...
			foreach ( $arrDayUserList as $val )
			{
                ...
                foreach ($GameList as $g) {
                	if ($val['KindID'] == $g['KindID']) {
						$arrDayUserList[$iCount]['KindName'] = $g['KindName'];
						break;
					}
				}
				$iCount++;
