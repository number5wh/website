...
class UserBankOutAction extends PageBase
{
    ...
    public function index()
    {
        ...
        $this->smarty->display($this->arrConfig['skin'].'/YunYing/UserBankOutList.html');
    }
    ...
    public function getPagerUserBankRate($pagesize){
        ...
        for($t = $STime; $t <= $ETime;$t = strtotime('+1 day',$t)){
            ...
            if($isExits['ret']===1) {
                //var_dump($subTable);
                if ($rateType == 1) {
                    $strWhere = ' WHERE ChangeType = 2';//转出
                } else {
...
//             }
    ...
    public function getBankChangepage()
    {
        ...
        $arrTags=array('skin'=>$this->arrConfig['skin'],'Page'=>$arrResult['Page'],'UserBankRateList'=>$arrResult['arrUserBankRateList'],"RoleID"=>$arrResult['RoleID']);
        Utility::assign($this->smarty,$arrTags);
        $html = $this->smarty->fetch($this->arrConfig['skin'].'/YunYing/UserBankOutList.html');
