...
class PHPZip{
	...
	//$autoDownload=true:自动下载,false:只生成不自动下载
	...
	* @param $override            是否覆盖
	*/
	function compressFilesToZip($zipFile,$files,$override = false)
	{	
		if (empty($zipFile) || empty($files) || gettype($files) != "array" && count($files)==0) 
		{
			throw new exception("parameters lose !");
		}
		
		if(!$override && is_file($zipFile)) 
		{
			unlink($zipFile);
		}
		...
		{
			throw new exception("create zip failed !");
		}
	}
