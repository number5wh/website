...
class OperationLogsDAL extends DALBase
{
	private $iRoleID = 0;
	public function __construct($iRoleID)
	{
		...
		parent::initDBObj($this->iRoleID,$this->arrConfig['MapType']['OperationLogs'],true);
	}
	...
	function getRecordsCount($arrParam)
	{
		...
		$arrResult = $this->objOperationLogsDB->fetchAssoc("Proc_GetRecordsCount", $params);
	...
	function getPageList($arrParam,$flag)
	{
		...
		if(!$arrResult)
		{
			...
			$arrResult = $this->objOperationLogsDB->fetchAllAssoc("Proc_GetPages", $params);
			if($flag)
	...
	function addCaseOperationLogs($iRoleID, $CaseSerial, $OpType, $OpContent, $OpResult, $ClientIP, $OpPlace, $TypeID, $SysUserName, $MachineSerial)
	{
		...
		$arrReturns = $this->objOperationLogsDB->fetchAssoc("Proc_OperationLogs_Insert", $params);	
		return $arrReturns;
	}
	...
	public function getOperationLogs($iRoleID, $CaseSerial)
	{
		...
		$arrReturns = $this->objOperationLogsDB->fetchAllAssoc("Proc_OperationLogs_Select", $params);		
		return $arrReturns;
	}
	...
	function addLockUserLogs($CaseSerial, $Reason, $Result, $LockExpireTime, $Requirement, $SysUserName, $Remarks)
	{
		...
		$arrReturns = $this->objOperationLogsDB->fetchAssoc("Proc_LockUserLogs_Insert", $params);		
		return $arrReturns;
	}
	/**
	 * 查询给定时间段大厅在线人数
	...
	function getHallOnlineUser($StartTime,$EndTime)
	{
	    ...
	    $arrReturns = $this->objOperationLogsDB->fetchAllAssoc("Proc_HallOnlineUser_Select",$params);
	    return $arrReturns;
	}
	/**
	 * 查询给定时间段房间在线人数
	...
	function getRoomOnlineUser($StartTime,$EndTime)
	{
	    ...
	    $arrReturns = $this->objOperationLogsDB->fetchAllAssoc("Proc_RoomOnlineUser_Select",$params);
	    return $arrReturns;
	}
	/**
	 * 删除管理员的操作日志
	...
	function deleteOperationLogs($SysUserName,$date)
	{
	    ...
	    $arrReturns = $this->objOperationLogsDB->fetchAllAssoc("Proc_OperationLogs_Delete",$params);
	    return $arrReturns;
	}
}
?>