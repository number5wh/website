...
class GamePostAction extends PageBase
{
    ...
    {
        $this->smarty->display($this->arrConfig['skin'].'/Service/GamePostList.html');
    }
    ...
    public function getPagerMsg()
    {
        ...
        $html = $this->smarty->fetch($this->arrConfig['skin'].'/Service/GamePostListPage.html');
        //echo $html;
    ...
    public function ShowAddMsgHtml()
    {
        ...
        $arrServerList = $objMasterBLL->getMsgInfo($MsgID, 2);
        ...
        Utility::assign($this->smarty,$arrTags);
        $html = $this->smarty->fetch($this->arrConfig['skin'].'/Service/GamePostEdit.html');
        $html=str_replace("</script>","<\/script>",str_replace("\r\n",'',$html));
        echo $html;
