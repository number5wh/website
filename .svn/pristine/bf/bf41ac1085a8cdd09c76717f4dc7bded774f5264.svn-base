...
class UserBankRateAction extends PageBase
{	
	...
	public function index()
	{
		...
		$this->smarty->display($this->arrConfig['skin'].'/YunYing/UserBankRateList.html');
	}	 
    ...
    public function getPagerUserBankRate($pagesize){
        ...
        $rateType = Utility::isNumeric('rateType',$_POST) ? $_POST['rateType']:1;//默认搜索游戏类型为2
        ...
        for($t = $STime; $t <= $ETime;$t = strtotime('+1 day',$t)){
            ...
            if($rateType == 1){
                ...
                $strWhere = ' WHERE ChangeType = 5';//          转出
            }
            if($RoleID){
        ...
        $arrParam['tableName'] = ' (SELECT TOP 1000 SUM(ChangeMoney) totalMoney,MAX(RoleName) szRoleName,RoleID FROM '.$subQuery." GROUP BY RoleID ".$strOrder.") T";
...
//                }
	...
	public function getPagerUserBankRateList()
	{
		...
		Utility::assign($this->smarty,$arrTags);
		$html = $this->smarty->fetch($this->arrConfig['skin'].'/YunYing/UserBankRateListPage.html');
