...
require_once ROOT_PATH . 'Common/app_wxpay593/lib/WxPay.Notify.php';
...
// 初始化日志
$log_path = dirname ( $_SERVER ["DOCUMENT_ROOT"] ) . "/logs/app_notify593/";
...
class PayNotifyCallBack extends WxPayNotify {
	...
	public function NotifyProcess($data, &$msg) {
		...
		$data = ( array ) $data;
		// 查询商户订单号是否存在;
		if (OSGetPayOrder ( $data ['out_trade_no'], $CardType )['iResult'] == - 1) {
			$msg = "订单查询失败";
			return false;
		}
