...
{	
    ...
    public function getPagerUserBankRate($pagesize){
        ...
        $SuperUser = 0;//Utility::isNumeric('SuperUser',$_POST)? $_POST['SuperUser'] : 0;  0普通玩家   1 商人
        ...
        for($t = $STime; $t <= $ETime;$t = strtotime('+1 day',$t)) {
            ...
            if ($isExits['ret'] === 1) {
                ...
                $strWhere .=" and RoleId not in(select Roleid from OM_MasterDB.dbo.T_SuperUser)";
...
//        echo ' (SELECT TOP 1000 SUM(ChangeMoney) totalMoney,MAX(RoleName) szRoleName,RoleID,(select SUM(ChangeMoney)
...
//                }
                ...
                if(!empty($arrUserBankRateList[$iCount]['totalzz'])){
                    $arrUserBankRateList[$iCount]['totalzz'] = Utility::FormatMoney($val['totalzz']);
                }
                else
                {
                    $arrUserBankRateList[$iCount]['totalzz'] = 0;
                }
