...
class ServerDcAction extends PageBase
{	
	...
	public function __construct()
	{
		...
		$this->strServerType = 6;//服务器类型,0:数据库服务器,1、游戏服务器,2、登录服务器,3、下载服务器,4、版本服务器,5、银行服务器,6、中心服务器, 7.大厅服务器
	}
	public function index()
	{
		/*$arrRes = $this->getServerList();
		$arrTags=array('ServerList'=>$arrRes,'ClassName'=>'ServerDc','ServerTypeName'=>'DC应用');*/
		$arrTags=array('TabTagID'=>'ServerDc','IP'=>'','Port'=>0,'ServID'=>0,'ClassName'=>'ServerDc');
	...
	public function showAddServerHtml()
	{
		...
		$arrTags=array('ServerTypeName1'=>'DC','ServerTypeName'=>'DC应用','ClassName'=>'ServerDc','server'=>$arrRes,'MustWriteTags'=>'');
		Utility::assign($this->smarty,$arrTags);
	...
	public function setServerLocked()
	{
		...
		if($iResult==0)
	 		...
	 		$msg=Utility::echoResultHtml($this->smarty,'取 消','main.CloseMsgBox(false,\'ServerDc\')','DC服务器配置信息发布失败','false','ServerGame',$this->arrConfig);	
	...
	public function delServer()
	{
		...
		if($ServerID && $ServerID>0)		
		{
			...
			if($iResult==0)
				...
				$html=Utility::echoResultHtml($this->smarty,'取 消','main.CloseMsgBox(false,\'ServerDc\')','删除失败,请重试','false','ServerGame',$this->arrConfig);
		}
		else 
			$html=Utility::echoResultHtml($this->smarty,'取 消','main.CloseMsgBox(false,\'ServerDc\')','对不起,您提交的数据异常,请重试','false','ServerGame',$this->arrConfig);
