...
class ServiceCheckAction extends PageBase
{
	...
	function index()
	{
		...
		$this->smarty->display($this->arrConfig['skin'].'/Service/AuthCheckIndex.html');
	}
	/**
	 * 获取操作审核页面
	 */
	...
	public function authVerifyList()
	{
			...
			if(is_array($arrResult) && count($arrResult)>0){
				...
				foreach ($arrResult as $v){										
					...
					if($v['OperationType']==10){
						...
						$arrResult[$i]['Amount'] = Utility::FormatMoney($v['iNumber']).$this->arrConfig['TreasureCompensateType'][7];
					}else{
						$arrResult[$i]['Amount'] =Utility::FormatMoney($v['iNumber']).$this->arrConfig['TreasureCompensateType'][$v['OperationType']];
					}
		...
		Utility::assign($this->smarty,$arrTags);		
		$html = $this->smarty->fetch($this->arrConfig['skin'].'/Service/AuthCenterList.html');
	...
	public function addApplyTreasure()
	{
		...
		}else
		{
			...
			$arrResult = $systemBLL->addAuthProcess($iRoleID,$iLoginId,$strLoginName,$iOperateType,$iExtId,$NewiNumber,'',$strReason,$strRemark,'',2,$this->strLoginedUser,0);
