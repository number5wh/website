...
class SysBankLogsAction extends PageBase
{	
	...
	public function index()
	{
		...
		$this->smarty->display($this->arrConfig['skin'].'/YunWei/SysBankLogsList.html');
	}	 
	...
	{
		$arrLogsList = null;
		$strWhere = " WHERE OpType=26";// AND OpContent LIKE '%".Utility::utf8ToGb2312('系统玩家')."%'";
	...
	{
		$arrResult = $this->getPagerLogs(10);
		$arrTags=array('skin'=>$this->arrConfig['skin'],'Page'=>$arrResult['Page'],'LogsList'=>$arrResult['arrLogsList']);
		Utility::assign($this->smarty,$arrTags);
		$html = $this->smarty->fetch($this->arrConfig['skin'].'/YunWei/SysBankLogsListPage.html');
