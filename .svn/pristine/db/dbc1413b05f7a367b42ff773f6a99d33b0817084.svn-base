...
class WechatUserAction extends PageBase
{
    ...
    public function getPagerServer()
    {
        ...
        $arrParam['fields']='id,type,weixinid,weixinname,noticetip,ShowWay';
        $arrParam['tableName']='T_WeiXinProxy';
        $arrParam['where']=' ';
        $arrParam['order']='type';
        $arrParam['pagesize']=25;
        ...
        if(is_array($arrServerList) && count($arrServerList)>0)
        {
            ...
            foreach($arrServerList as $key => $val)
            {
                ...
                else if($arrServerList[$key]['type']==4)
                {
                    $arrServerList[$key]['typename']="举报有奖";
                }
                $arrServerList[$key]['weixinname']=str_replace("\n","",Utility::gb2312ToUtf8($arrServerList[$key]['weixinname']));
				$arrServerList[$key]['noticetip']=str_replace("\n","",Utility::gb2312ToUtf8($arrServerList[$key]['noticetip']));
            }
        }
    ...
    function topuser(){
        ...
        if($WechatID && $WechatID>0)
        {
            $iResult = $this->objMasterBLL->topWechatUser($WechatID);
            $iResult=$iResult[0]["iResult"];
            if($iResult==0)
                $html='';
            else
                $html=Utility::echoResultHtml($this->smarty,'取 消','main.CloseMsgBox(false,\'ServerGame\')','删除失败,请重试','false','ServerGame',$this->arrConfig);
