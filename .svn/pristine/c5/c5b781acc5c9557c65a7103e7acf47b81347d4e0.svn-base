...
class AndroidHallVersionAction extends PageBase
{	
	...
	public function __construct()
	{
		...
		$this->iVerType = 4; //版本类型(1:游戏种类版本,2:大厅版本,3:道具版本，4安卓端版本)
		$this->iKindID = 0;
	}
	...
	{		
		$this->smarty->display($this->arrConfig['skin'].'/YunWei/AndroidHallList.html');
	}	
	/**
	 * 显示添加版本页面
	 */
	public function showAddAndroidHallVersioniHtml()
	{		
		...
		$arrTags = array('ServerList'=>$arrServer,'KindID'=>$this->iKindID,'GameVersion'=>$arrVersion,'skin'=>$this->arrConfig['skin']);
		Utility::assign($this->smarty,$arrTags);
		$html = $this->smarty->fetch($this->arrConfig['skin'].'/YunWei/AndroidHallVerstion.html');
	 ...
	 * 分页
	 */
	public function getPagerAndroidHall()
	{
		...
		Utility::assign($this->smarty,$arrTags);
		$html = $this->smarty->fetch($this->arrConfig['skin'].'/YunWei/AndroidHallListPage.html');
		$html=str_replace("</script>","<\/script>",str_replace("\r\n",'',$html));
		echo $html;
	}
	/**
