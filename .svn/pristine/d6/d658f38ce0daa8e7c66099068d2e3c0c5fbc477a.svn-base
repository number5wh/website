...
class ServerLoginAction extends PageBase
{	
	...
	public function index()
	{
		...
		$arrTags=array('ServerList'=>$arrRes,'ClassName'=>'ServerLogin','ServerTypeName'=>'下载应用');*/
		$arrTags=array('TabTagID'=>'ServerLogin','IP'=>'','Port'=>0,'ServID'=>0,'ClassName'=>'ServerLogin');
	...
	public function showAddServerHtml()
	{
		...
		$arrTags=array('ServerTypeName1'=>'服务器','ServerTypeName'=>'登陆应用','ClassName'=>'ServerLogin','server'=>$arrRes,'MustWriteTags'=>'');
		Utility::assign($this->smarty,$arrTags);
	...
	public function setServerLocked()
	{
		...
		if($iResult==0)
	 		...
	 		$msg=Utility::echoResultHtml($this->smarty,'取 消','main.CloseMsgBox(false,\'ServerLogin\')','登陆服务器配置信息发布失败','false','ServerLogin',$this->arrConfig);	
	...
	public function delServer()
	{
		...
		if($ServerID && $ServerID>0)		
		{
			...
			if($iResult==0)
				...
				$html=Utility::echoResultHtml($this->smarty,'取 消','main.CloseMsgBox(false,\'ServerLogin\')','删除失败,请重试','false','ServerLogin',$this->arrConfig);
		}
		else 
			$html=Utility::echoResultHtml($this->smarty,'取 消','main.CloseMsgBox(false,\'ServerLogin\')','对不起,您提交的数据异常,请重试','false','ServerLogin',$this->arrConfig);
